    ...
    publish(articleForm) {
      ...
      this.$refs[articleForm].validate((valid) => {
        if (valid) {
          ...
          formData.append('coverImage', this.articleForm.coverImage);

          publishArticle(formData, this.$store.state.token)
            .then((data) => {
              if (data.success) {
                loading.close();
